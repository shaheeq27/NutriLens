...
# Database's per-100g banana record (real values used elsewhere in this project).
_BANANA_PER_100G = DatabaseNutrients(
...
class _FailingProvider:
    def lookup_food(self, food_name: str):
        raise NutritionLookupError("simulated Database outage")


def test_scaling_matches_hand_calculated_values_for_a_118g_banana():
    provider = MockNutritionLookupProvider(
        DatabaseFoodMatch(db_id=1105073, description="Bananas, raw", nutrients=_BANANA_PER_100G)
    )
    ...
    assert result.db_id == 1105073


def test_scaling_at_exactly_100g_leaves_values_unchanged():
    provider = MockNutritionLookupProvider(
        DatabaseFoodMatch(db_id=1, description="Test food", nutrients=_BANANA_PER_100G)
    )
...
def test_missing_nutrient_fields_stay_none_after_scaling():
    ...
    provider = MockNutritionLookupProvider(
        DatabaseFoodMatch(db_id=1, description="Bananas, raw", nutrients=_BANANA_PER_100G)
    )
...
def test_not_found_outcome():
    provider = MockNutritionLookupProvider(
        DatabaseLookupRejection(reason=DatabaseLookupReason.NO_MATCH, message="no match")
    )
...
def test_provider_error_is_captured_not_raised():
    ...
    assert "simulated Database outage" in result.message


def test_zero_or_negative_portion_raises_value_error():
    provider = MockNutritionLookupProvider(
        DatabaseFoodMatch(db_id=1, description="Test", nutrients=_BANANA_PER_100G)
    )
